...
<HTML>
<HEAD>
<TITLE>StrawHat 2011 Staff Tech Design Information</TITLE>
...
$check_result = mysql_query($checkname,$connection) or die("<p>Couldn't execute CheckName query. If the error persists, take out any slashes, apostrophes, quotes, etc. and try again. Please report the error to us at jay@strawhat-auditions.com</p>");
	...
	elseif(mysql_num_rows($check_result) !=0) {
		...
		echo "<p>The Staff Tech Design name $c_first $c_mid $c_last at $email already exists. If you are trying to change or complete your application, please use the <a href=\"/TechEntry11/profile_change/techie_searchlastname.php\">change Staff Tech Design link</a>. If you are just starting an application and this name is already being used, please use another name or change it with an initial or middle name.<br><br>
...
/*DID TECH INSERT SUCCEED*/
	if (!$sql_result) {
	echo "<P>Couldn't add TECH record! Use your back button to go back to the Staff Tech Design entry and try again.</P>";
	exit();
	}
//----------------------------------------------------------------------------//
...
$sql_result = mysql_query($sql_app,$connection) or die("Couldn't execute S/T/D Application query.");
	
	if (!$sql_result) {
	echo "<P>Couldn't add S/T/D Application record!</P>";
	} 
	
//execute SQL PHYSICAL query and get result
$sql_result = mysql_query($sql_roles,$connection) or die("Couldn't execute S/T/D Roles query.");
	
	if (!$sql_result) {
	echo "<P>Couldn't add S/T/D Roles record!</P>";
	} 
