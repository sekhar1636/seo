...
//create sql statement, includes ALL ACTORS
$sqlA ="SELECT skills_uid, actor_uid, audition_uid, actor_ID, phys_uid, pix_link,
...
gender, nativeam, asian, white, black, hispanic, eeurope, mideast, indian, mononly
...
if($day) {$sql = $sql . $sql_day;}
if($time) {$sql = $sql . $sql_time;}
if($type) {$sql = $sql . $sql_type;}
if($standby) {$sql = $sql . $sql_standby;}
$sql = $sql . $sql_B;
echo "$sql";
...
$sql_result = mysql_query($sql,$connection) or die("Couldn't execute Advanced Search query.");
